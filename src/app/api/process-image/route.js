...
const sendToTelegram = async (
  ...
  try {
    ...
    if (isSuccess) {
      console.log(
        chalk.blue(`✉️ [后台] 发送图片 URL 到 Telegram: ${TELEGRAM_CHAT_ID}`),
      );
      ...
      console.log(
        chalk.blue(`✉️ [后台] 发送错误消息到 Telegram: ${TELEGRAM_CHAT_ID}`),
      );
...
async function processImageInBackground(
  ...
  try {
    ...
    console.log(
      chalk.green(`✅ 找到图像 URL: ${imageUrl.substring(0, 100)}...`),
    );
  ...
  queue
    .add(async () => {
      const promptSnippet = finalPromptToUse
        ? `(${finalPromptToUse.substring(0, 30)}...)`
        : '(仅图片)';
...
export async function POST(req) {
  ...
  try {
    ...
    switch (receivedPromptType) {
      ...
      case PromptType.CUSTOM:
        if (!customPromptTextFromRequest?.trim()) {
          console.error(
            chalk.red(`❌ 收到 'custom' 类型但未收到有效的 'customPromptText'`),
          );
          return NextResponse.json(
    ...
    const safeOriginalFilename = path
      ...
      .replace(/[^a-zA-Z0-9.\-_]/g, '_'); // Basic sanitize
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || '处理上传时发生内部服务器错误。',
      },
      { status: 500 },
    );
  }
